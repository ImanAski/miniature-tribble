...
static uint8_t s_seq_counter = 0; /* Auto-incremented for device-originated events */
...
{
    if (!plat || !plat->write_bytes) return;

    /* Guard against oversized payloads */
    if (payload_len > DM_MAX_PAYLOAD) payload_len = DM_MAX_PAYLOAD;
